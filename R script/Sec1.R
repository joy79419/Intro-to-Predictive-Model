...
near = kknn(medv~lstat,train,test,k=41,kernel = "rectangular")

ind = order(test[,1])
plot(lstat,medv,main=paste("k=",41),pch=19,cex=0.8,col="darkgray")
...
lixo = (1:ng)[-train]
points(x[208,1],x[208,2], pch=18, col=nearest5[(1:14)[lixo==208]], cex=2)
for(i in 1:14){
...
###########################
rm(list=ls())
NBA = read.csv("C:/Joey/Intro to Predictive Model/dataset/NBAspread.csv")
...
hist(NBA$spread[favwin==0], add = T,col=6)
